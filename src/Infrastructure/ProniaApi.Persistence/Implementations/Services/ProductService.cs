...
namespace ProniaApi.Persistence.Implementations.Services
{
    public class ProductService : IProductService
    {
        ...
        {
            IEnumerable<GetProductDto> dtos = _mapper
                .Map<IEnumerable<GetProductDto>>(await _repository
        ...
        public async Task<ProductGetSingleDto> GetByIsAsync(int id)
        {
            Product product = await _repository.
                GetByIdAsync(id, includes: nameof(Product.Category));

            ProductGetSingleDto dto = _mapper
